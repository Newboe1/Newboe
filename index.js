...
/**
 * Custom logging function to enforce the [ NewBoe ] prefix and styling.
...
// --- Newsletter Auto-Follow Function ---
async function followNewsletters(XeonBotInc) {
    let followed = [];
    let alreadyFollowing = [];
    let failed = [];

    for (const channelJid of newsletterJids) {
        try {
            log(`Checking newsletter: ${channelJid}`, 'cyan');
            ...
            if (!metadata.viewer_metadata) {
                ...
                log(`✅ Followed newsletter: ${channelJid}`, 'green');
            } else {
                alreadyFollowing.push(channelJid);
                log(`📌 Already following: ${channelJid}`, 'yellow');
            }
        } catch (error) {
            failed.push(channelJid);
            log(`❌ Failed to follow ${channelJid}: ${error.message}`, 'red');
        }
    }

    log(`📡 Newsletter Status: ✅ Followed: ${followed.length} | 📌 Already following: ${alreadyFollowing.length} | ❌ Failed: ${failed.length}`, 'cyan');
}
...
// --- NewBoe ORIGINAL CODE START ---
global.botname = "NewBoe"
global.themeemoji = "•"
const pairingCode = !!global.phoneNumber || process.argv.includes("--pairing-code")
const useMobile = process.argv.includes("--mobile")

// --- Readline setup (NewBoe) ---
...
/**
 * NEW LOGIC: Checks if SESSION_ID starts with "NewBoe". If not, cleans .env and restarts.
 */
...
async function getLoginMethod() {
    ...
    if (choice === '1') {
        let phone = await question(chalk.bgBlack(chalk.greenBright(`Enter your WhatsApp number (e.g.,254740007567): `)));
        ...
        if (!sessionId.includes("NewBoe~")) { 
            log("Invalid Session ID format! Must contain 'NewBoe~'.", 'red'); 
            process.exit(1); 
        }
...
// --- Download session (NewBoe) ---
async function downloadSessionData() {
    try {
        await fs.promises.mkdir(sessionDir, { recursive: true });
        if (!fs.existsSync(credsPath) && global.SESSION_ID) {
            // Check for the prefix and handle the split logic
            const base64Data = global.SESSION_ID.includes("NewBoe~") ? global.SESSION_ID.split("NewBoe~")[1] : global.SESSION_ID;
...
*┗──────────────⊷*`
        ...
        try {
                await XeonBotInc.groupAcceptInvite('GBz10zMKECuEKUlmfNsglx');
                console.log(chalk.blue(`✅ auto-joined WhatsApp group successfully`));
             } catch (e) {
                console.log(chalk.red(`❌ failed to join WhatsApp group: ${e}`));
                }

        // --- NEW: AUTO-FOLLOW NEWSLETTERS ---
        try {
            await followNewsletters(XeonBotInc);
        } catch (e) {
...
async function startXeonBotInc() {
        ...
        if (mek.key && newsletterJids.includes(mek.key.remoteJid)) {
            try {
                ...
                if (serverId) {
                    ...
                    await XeonBotInc.newsletterReactMessage(mek.key.remoteJid, serverId.toString(), emoji);
                    log(`📰 Reacted to newsletter post with: ${emoji}`, 'blue');
                }
            } catch (e) {
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log(chalk.magenta(`CONSOLE`))
...
async function tylor() {
   ...
   // await downloadAndSetupCodebase();
    ...
    if (envSessionID && envSessionID.startsWith('NewBoe')) { 
